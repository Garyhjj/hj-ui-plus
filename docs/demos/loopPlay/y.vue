...
import { HjLoopPlayInstance } from '@hj-ui-plus/components'
...
const playBoxHeight = ref('710px')
const player = ref<HjLoopPlayInstance>()
...
const updateAndPlay = (scale) => {
  playBoxHeight.value = 710 * scale + 'px'
  player.value!.initAndStart()
}
</script>
...
.loop-box::before {
  ...
  height: 56px;
  width: 100%;
  background: linear-gradient(180deg, #0a2540 0%, rgba(10, 37, 64, 0) 50%);
  z-index: 9;
}
...
.loop-box::after {
  ...
  height: 56px;
  width: 100%;
  background: linear-gradient(0deg, #0a2540 0%, rgba(10, 37, 64, 0) 50%);
  z-index: 9;
}
